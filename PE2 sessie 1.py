...
true_measurements_f = []
f_true = np.mean(true_measurements_f)
f_std = np.std(true_measurements_f)


def correct_condition(mu, sigma, f_true):
    if np.abs(mu - f_true) + 2 * sigma <= 0.01 * f_true:
        return True
    else:
        return False

# other measurements
measurements_f = np.array([[], []])
means, stds = [], []
correct = []
for group_measurements in measurements_f:
    mean_f = np.mean(group_measurements)
    std_f = np.std(group_measurements)
    condition = correct_condition(mean_f, std_f, f_true)
    means.append(mean_f)
    stds.append(std_f)
    correct.append(condition)
    
# measurements = range(len(measurements_f))
# plt.errorbar(measurements, means, yerr=stds)
# plt.axhline(f_true, min(measurements), max(measurements))
x_axis = np.arange(len(measurements_f)) 

plt.errorbar(x_axis, means, yerr=stds, fmt='o', capsize=5, label='Measured')
plt.axhline(y=f_true, color='r', linestyle='--', label='True Value')
plt.show()

# aliasing
# f_sampling = 1 # sampling rate
# n = f_signal / f_sampling
# measured_prediction = 

def predict_aliasing(f_signal, f_s):
    """
    Predicts the measured frequency due to aliasing.
    Based on: f_measured = |± f_signal ± n * f_s|
    """
    # Equation 10: n is the integer closest to the ratio
    n = round(f_signal / f_s)
    
    # Equation 9: Possible combinations of ±f_signal ± n*f_s
    # We typically look for the value in the range [0, f_s/2]
    ...
    # The actual aliased frequency is the one that falls 
    # within the Nyquist window (0 to f_s/2)
    f_predicted = min(possible_frequencies)
    
    return f_predicted

# Example usage within your loop logic:
f_s = 1000  # Example sampling frequency
f_signal = 1200 # Example true signal frequency

f_pred = predict_aliasing(f_signal, f_s)
print(f"Predicted Aliased Frequency: {f_pred} Hz")




#%%
def theoretical_step_function(t, V_0, omega):
    return V_0 * (1 - np.exp(-t / omega))

y_data = []

params, covariance = curve_fit(theoretical_step_function, t, y_data, p0=[1, 1])

V_0_opt, omega_opt = params
print(f"Voltage: {V_0_opt}, Constant: {omega_opt}")

y_pred = theoretical_step_function(t, V_0_opt, omega_opt)
